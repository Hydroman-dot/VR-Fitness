...
DEFAULT_CONFIG = {
    ...
    "health_show_spo2": True,
    "health_use_weight": True,
    "health_send_active_calories": True,
    "profile_weight_kg": "",
    "profile_age": "",
    "profile_sex": "unspecified",
...
TRANSLATIONS_EN = {
    ...
    "HERZFREQUENZ": "HEART RATE", "GESCHWINDIGKEIT": "SPEED", "GESAMT": "TOTAL", "Schritte": "steps", "AKTIVE KCAL": "ACTIVE KCAL",
    ...
    "Pulsoid Widget URL:": "Pulsoid Widget URL:",
    "Pulsoid Widget URL (experimentell)": "Pulsoid Widget URL (experimental)",
    "Pulsoid Widget URL nutzt den öffentlichen Widget-Link ohne eigenen API-Token in VR Fitness. Diese Anbindung ist inoffiziell/experimentell und kann sich ändern.": "Pulsoid Widget URL uses the public widget link without storing a separate API token in VR Fitness. This integration is unofficial/experimental and may change.",
    "Beim Start automatisch nach Updates suchen": "Automatically check for updates on startup",
    "Update verfügbar": "Update available", "Updateprüfung": "Update check",
    "Installiert": "Installed", "Verfügbar": "Available", "Änderungen": "Changes",
    "Eine neuere Version ist verfügbar.": "A newer version is available.",
    "Jetzt aktualisieren": "Update now", "Später erinnern": "Remind me later",
    "Diese Version überspringen": "Skip this version",
    "Diese Version wird nicht mehr automatisch angeboten. Eine neuere Version wird wieder angezeigt.": "This version will no longer be offered automatically. A newer version will be shown again.",
    "Du bist auf dem aktuellen Stand.": "You are up to date.",
    "Keine Update-Quelle konfiguriert.": "No update source configured.",
...
def estimate_active_calories_keytel(rows, weight_kg, age_years, sex):
    """
    Experimental exercise-energy estimate based on the Keytel et al. (2005)
    heart-rate equation without VO2max. Integrates cleaned HR samples over time.
    Result is an estimate only, not a medical measurement.
    """
    try:
    ...
    if sex not in ("male", "female"):
        return None
    if not (20 <= weight <= 300 and 12 <= age <= 100):
        return None

    points = []
    for r in rows:
        t = r.get("timestamp")
        hr = safe_float(r.get("clean_bpm"))
        if t is not None and hr is not None and 35 <= hr <= 230:
            points.append((t, hr))
    ...
    kcal = 0.0
    for (t0, hr0), (t1, _) in zip(points, points[1:]):
        ...
        kcal_per_min = max(0.0, kj_per_min / 4.184)
        kcal += kcal_per_min * (dt_s / 60.0)

    if not math.isfinite(kcal) or kcal <= 0:
        return None
    return min(kcal, 9999.0)
...
def _pulsoid_widget_get_read_token(widget_url):
    """
    Experimental Pulsoid widget-link reader.
    The widget itself is public via its unique URL. This helper asks Pulsoid's
    public widget RPC for the temporary read token used by the browser widget.
    This endpoint is not part of the documented public API and may change.
    """
    widget_url = (widget_url or "").strip()
    prefix = "https://pulsoid.net/widget/view/"
    if not widget_url.startswith(prefix):
        raise ValueError(
            "Ungültige Pulsoid Widget URL. Erwartet: "
            "https://pulsoid.net/widget/view/..."
        )

    widget_id = widget_url[len(prefix):].split("?", 1)[0].split("#", 1)[0].strip("/")
    if not widget_id:
        raise ValueError("Pulsoid Widget-ID fehlt.")
    ...
    req = urllib.request.Request(
        "https://pulsoid.net/v1/api/public/rpc",
        data=payload,
        method="POST",
        headers={
    ...
    if not token:
        raise RuntimeError("Pulsoid Widget konnte keinen Lesetoken liefern.")
    return str(token)


async def pulsoid_widget_client(widget_url, data, stop_event, log):
    """
    Experimental tokenless setup for the user:
    the user pastes a Pulsoid browser-widget URL instead of an API token.
    A temporary read token is obtained from the public widget endpoint.
    """
    data.pulse_source_name = "Pulsoid Widget"
    widget_url = (widget_url or "").strip()

    if not widget_url:
        data.pulse_ws_state = "disconnected"
        log("[Pulsoid Widget] Keine Widget-URL hinterlegt.")
        while not stop_event.is_set():
            await asyncio.sleep(2)
        return
    ...
    while not stop_event.is_set():
        ...
        data.pulse_ws_connected = False
        try:
            log("[Pulsoid Widget] Lese Widget-Verbindung ...")
            token = await asyncio.to_thread(
                _pulsoid_widget_get_read_token,
                widget_url,
            )
            ws_url = (
                "wss://dev.pulsoid.net/api/v1/data/real_time"
            ...
            log("[Pulsoid Widget] Verbinde Realtime-WebSocket ...")
            async with websockets.connect(
                ws_url,
                ping_interval=20,
                ping_timeout=20,
            ...
            if not stop_event.is_set():
                log(
                    "[Pulsoid Widget] Verbindung getrennt: "
                    f"{exc.__class__.__name__}: {exc}. "
                    f"Neuer Versuch in {backoff}s."
                )
        finally:
            data.pulse_ws_connected = False
            if not stop_event.is_set():
...
class ConnectionEngine:
    ...
    async def _async_main(self, settings):
        ...
        elif pulse_source == "Pulsoid Widget URL":
            pulse_task = asyncio.create_task(
                pulsoid_widget_client(
                    settings.get("pulsoid_widget_url", ""),
                    self.data,
                    self.shutdown_event,
                    self.gui.log,
                )
...
def summary_to_text(summary):
    lines = [
        ...
        "ENERGIE (SCHÄTZUNG)",
        "-" * 58,
        f"Aktive Kalorien: {de_number(summary.get('active_calories_kcal'), 0)} kcal",
        f"Methode: {summary.get('calorie_estimate_method') or '--'}",
        f"Gewicht: {de_number(summary.get('calorie_weight_kg'), 1)} kg"
        + (f" ({summary.get('calorie_weight_source')})" if summary.get('calorie_weight_source') else ""),
        "",
        "VERBINDUNGSQUALITÄT",
        "-" * 58,
...
class App:
    def __init__(self, root):
        ...
        self.pulsoid_widget_url = tk.StringVar(
            value=self.cfg.get("pulsoid_widget_url", "")
        )
        ...
        self.update_skipped_version = str(self.cfg.get("update_skipped_version", "")).strip()
        ...
        self.health_show_spo2 = tk.BooleanVar(
            value=bool(self.cfg.get("health_show_spo2", True))
        )
        self.health_use_weight = tk.BooleanVar(
            value=bool(self.cfg.get("health_use_weight", True))
        )
        self.health_send_active_calories = tk.BooleanVar(
            value=bool(self.cfg.get("health_send_active_calories", True))
        )
        self.profile_weight_kg = tk.StringVar(value=str(self.cfg.get("profile_weight_kg", "")))
        self.profile_age = tk.StringVar(value=str(self.cfg.get("profile_age", "")))
        self.profile_sex = tk.StringVar(value=str(self.cfg.get("profile_sex", "unspecified")))
        ...
        self.health_weight_kg = None
        self.health_weight_updated_ms = None
        self.health_spo2_pct = None
        self.health_spo2_updated_ms = None
    ...
    def build_ui(self):
        ...
        kcal_box = ttk.Frame(metric_grid, style="Card2.TFrame")
        kcal_box.grid(row=2, column=0, columnspan=2, sticky="nsew", pady=(8, 0))
        ttk.Label(
            kcal_box, text=self.tr("AKTIVE KCAL"),
            background=c["panel2"], foreground=c["muted"],
            font=("Segoe UI Semibold", 8)
        ).pack(anchor="w", padx=12, pady=(10, 2))
        self.status_session_kcal = ttk.Label(
            kcal_box,
            text="-- kcal  ·  geschätzt",
            background=c["panel2"], foreground=c["text"],
            font=("Segoe UI Semibold", 15)
        )
        self.status_session_kcal.pack(anchor="w", padx=12, pady=(0, 10))
    ...
    def connection_settings(self):
        return {
            ...
            "pulsoid_widget_url": self.pulsoid_widget_url.get().strip(),
    ...
    def show_settings(self):
        ...
        ttk.Label(sources, text=self.tr("Pulsoid Widget URL:")).grid(row=9, column=0, sticky="w", **pad)
        ttk.Entry(sources, textvariable=self.pulsoid_widget_url, width=55).grid(row=9, column=1, sticky="ew", **pad)
        ttk.Label(
            sources,
            text=self.tr("Pulsoid Widget URL nutzt den öffentlichen Widget-Link ohne eigenen API-Token in VR Fitness. Diese Anbindung ist inoffiziell/experimentell und kann sich ändern."),
            foreground="#666666",
            wraplength=620,
        ).grid(row=10, column=0, columnspan=2, sticky="w", padx=10, pady=(0, 8))

        ttk.Label(sources, text="HypeRate Device-ID:").grid(row=11, column=0, sticky="w", **pad)
        ttk.Entry(sources, textvariable=self.hyperate_device_id, width=55).grid(row=11, column=1, sticky="ew", **pad)

        ttk.Label(sources, text="HypeRate WebSocket-Key:").grid(row=12, column=0, sticky="w", **pad)
        ttk.Entry(sources, textvariable=self.hyperate_api_key, show="•", width=55).grid(row=12, column=1, sticky="ew", **pad)
        ...
        ).grid(row=14, column=0, columnspan=2, sticky="w", padx=10, pady=10)
        ...
        ttk.Checkbutton(
            health,
            text="SpO₂ von Health Connect auf der Hauptseite anzeigen",
            variable=self.health_show_spo2
        ).grid(row=7, column=0, columnspan=2, sticky="w", **pad)

        ttk.Checkbutton(
            health,
            text="Aktuelles Health-Connect-Gewicht für Kalorienschätzung verwenden",
            variable=self.health_use_weight
        ).grid(row=8, column=0, columnspan=2, sticky="w", **pad)

        ttk.Checkbutton(
            health,
            text="Geschätzte aktive Kalorien an Health Connect senden",
            variable=self.health_send_active_calories
        ).grid(row=9, column=0, columnspan=2, sticky="w", **pad)

        ttk.Label(health, text="Gewicht manuell [kg]:").grid(row=10, column=0, sticky="w", **pad)
        ttk.Entry(health, textvariable=self.profile_weight_kg, width=12).grid(row=10, column=1, sticky="w", **pad)

        ttk.Label(health, text="Alter [Jahre]:").grid(row=11, column=0, sticky="w", **pad)
        ttk.Entry(health, textvariable=self.profile_age, width=12).grid(row=11, column=1, sticky="w", **pad)

        ttk.Label(health, text="Geschlecht für Kalorienformel:").grid(row=12, column=0, sticky="w", **pad)
        ttk.Combobox(
            health,
            textvariable=self.profile_sex,
            state="readonly",
        ...
        ttk.Label(
            health,
            text="Kalorien sind ein Schätzwert aus Gewicht, Alter und bereinigtem Pulsverlauf (Keytel 2005).",
            foreground="#666666",
            wraplength=620
        ...
        ttk.Checkbutton(
            diag,
            text=self.tr("Beim Start automatisch nach Updates suchen"),
            variable=self.update_check_on_start
        ).pack(anchor="w", padx=12, pady=(10, 2))
    ...
    def save_settings(self):
        ...
        cfg.update({
            ...
            "health_show_spo2": bool(self.health_show_spo2.get()),
            "health_use_weight": bool(self.health_use_weight.get()),
            "health_send_active_calories": bool(self.health_send_active_calories.get()),
            "profile_weight_kg": self.profile_weight_kg.get().strip(),
            "profile_age": self.profile_age.get().strip(),
            "profile_sex": self.profile_sex.get().strip(),
    ...
    def _finish_recording_gui(self, path):
        try:
            ...
            self.enrich_summary_with_health_estimates(path, summary)
    ...
    def _save_skipped_update(self, version):
        self.update_skipped_version = str(version or "").strip()
        self.cfg["update_skipped_version"] = self.update_skipped_version
        save_config(self.cfg)

    def _show_update_choice(self, latest, download, notes):
        win = tk.Toplevel(self.root)
        win.title(self.tr("Update verfügbar"))
        win.transient(self.root)
        ...
        ttk.Label(
            body,
            text=self.tr("Update verfügbar"),
            font=("Segoe UI", 15, "bold")
        ).pack(anchor="w", pady=(0, 10))

        info = (
            f"{self.tr('Installiert')}: {APP_VERSION}\n"
            f"{self.tr('Verfügbar')}: {latest}\n\n"
            f"{self.tr('Eine neuere Version ist verfügbar.')}"
        )
        ...
        ttk.Separator(body, orient="horizontal").pack(fill="x", pady=(8, 12))
        ...
        def update_now():
            ...
            if download:
                try:
                    ...
                    messagebox.showerror(self.tr("Updateprüfung"), str(exc))
            else:
                messagebox.showinfo(self.tr("Update verfügbar"), info)

        def remind_later():
            win.destroy()

        def skip_this_version():
            self._save_skipped_update(latest)
            win.destroy()
            messagebox.showinfo(
                self.tr("Update verfügbar"),
                self.tr("Diese Version wird nicht mehr automatisch angeboten. Eine neuere Version wird wieder angezeigt.")
            )

        ttk.Button(
            actions,
            text=self.tr("Jetzt aktualisieren"),
            command=update_now
        ).pack(side="left", padx=(0, 8))
        ttk.Button(
            actions,
            text=self.tr("Später erinnern"),
            command=remind_later
        ).pack(side="left", padx=8)
        ttk.Button(
            actions,
            text=self.tr("Diese Version überspringen"),
            command=skip_this_version
        ).pack(side="right", padx=(8, 0))

        win.protocol("WM_DELETE_WINDOW", remind_later)
    ...
    def check_for_updates(self, silent=False):
        ...
        if not url:
            if not silent:
                messagebox.showerror(self.tr("Updateprüfung"), self.tr("Keine Update-Quelle konfiguriert."))
            return

        def show_result(latest, download, notes):
            if self._version_tuple(latest) > self._version_tuple(APP_VERSION):
                # "Diese Version überspringen" unterdrückt nur automatische Hinweise.
                # Eine manuelle Prüfung darf die Version weiterhin anzeigen.
                if silent and latest == self.update_skipped_version:
                    return
                self._show_update_choice(latest, download, notes)
            elif not silent:
                messagebox.showinfo(
                    self.tr("Updateprüfung"),
                    f"{self.tr('Installiert')}: {APP_VERSION}\n"
                    f"{self.tr('Verfügbar')}: {latest or '--'}\n\n"
                    f"{self.tr('Du bist auf dem aktuellen Stand.')}"
                )
            ...
            try:
                ...
                if not silent:
                    self.root.after(0, lambda: messagebox.showerror(
                        self.tr("Updateprüfung"),
                        f"{self.tr('Updateprüfung fehlgeschlagen:')}\n{exc}"
                    ))
    ...
    def show_diagnostics(self):
        ...
        lines = [
            ...
            f"Health-Gewicht: {de_number(self.health_weight_kg, 1)} kg",
            f"Health-SpO₂: {de_number(self.health_spo2_pct, 1)} %",
    ...
    def _apply_health_status_result(self, result, update_status_text=False):
        ...
        try:
            self.health_weight_kg = (
                float(result.get("weight_kg"))
        ...
        try:
            self.health_weight_updated_ms = (
                int(result.get("weight_updated_ms"))
                if result.get("weight_updated_ms") is not None
                else None
            )
        except (TypeError, ValueError):
            self.health_weight_updated_ms = None
        ...
        try:
            self.health_spo2_pct = (
                float(result.get("spo2_pct"))
                if result.get("spo2_pct") is not None
                else None
            )
        except (TypeError, ValueError):
            self.health_spo2_pct = None
        ...
        try:
            self.health_spo2_updated_ms = (
                int(result.get("spo2_updated_ms"))
                if result.get("spo2_updated_ms") is not None
                else None
            )
        except (TypeError, ValueError):
            self.health_spo2_updated_ms = None
        ...
        if update_status_text:
            ...
            if self.health_weight_kg is not None:
                parts.append(f"Gewicht {de_number(self.health_weight_kg, 1)} kg")
            if self.health_spo2_pct is not None:
                parts.append(f"SpO₂ {de_number(self.health_spo2_pct, 1)} %")
    ...
    def build_health_payload(self, csv_path, summary):
        ...
        # Health Connect does not need the local 1-second resolution.
        # Keep roughly four HR samples per minute (~15 s), while CSV remains full resolution.
        hr_samples = []
        last_hr_ts = None
        for r in rows:
            ts = r.get("timestamp")
            ...
            if ts is None or bpm is None:
                continue
            if last_hr_ts is None or (ts - last_hr_ts).total_seconds() >= 15.0:
                hr_samples.append({
        ...
        return {
            ...
            "active_calories_kcal": summary.get("active_calories_kcal")
                if self.health_send_active_calories.get() else None,
            "calorie_estimate_method": summary.get("calorie_estimate_method")
                if self.health_send_active_calories.get() else None,
    ...
    def enrich_summary_with_health_estimates(self, csv_path, summary):
        rows = read_csv(Path(csv_path))

        weight = None
        weight_source = None
        if self.health_use_weight.get() and self.health_weight_kg is not None:
            weight = self.health_weight_kg
            weight_source = "Health Connect"
        else:
            weight = safe_float(self.profile_weight_kg.get())
            if weight is not None:
                weight_source = "manuell"

        age = safe_float(self.profile_age.get())
        sex = self.profile_sex.get().strip().lower()

        kcal = estimate_active_calories_keytel(
            rows,
            weight,
            age,
            sex,
        ...
        summary["calorie_weight_kg"] = weight
        summary["calorie_weight_source"] = weight_source
        summary["calorie_age_years"] = age
        summary["calorie_sex"] = sex
        summary["active_calories_kcal"] = kcal
        summary["calorie_estimate_method"] = (
            "Keytel 2005 (Puls + Gewicht + Alter + Geschlecht)"
            if kcal is not None else None
        )
        return summary
    ...
    def show_recent_sessions(self):
        ...
        columns = ("date", "session", "duration", "distance", "pulse", "kcal")
        tree = ttk.Treeview(
            table_inner,
            columns=columns,
            show="headings",
        ...
        tree.column("kcal", width=90, minwidth=75, anchor="e")
        ...
        for item in sessions:
            ...
            iid = tree.insert(
                ...
                values=(
                    ...
                    f"{de_number(summary.get('active_calories_kcal'), 0)}" if summary.get("active_calories_kcal") is not None else "--",
                )
            )
    ...
    def show_history(self):
        ...
        total_kcal = sum((s.get("active_calories_kcal") or 0) for s in weekly)
        ...
        ttk.Label(
            ...
            text=(
                ...
                f"~{de_number(total_kcal, 0)} kcal"
            )
        ).pack(anchor="w", padx=10, pady=8)

        columns = ("date", "name", "duration", "active", "distance", "steps", "avg_bpm", "kcal")
        ...
        for s in summaries:
            ...
            iid = tree.insert(
                ...
                values=(
                    ...
                    de_number(s.get("active_calories_kcal"), 0),
                )
            )
    ...
    def estimate_live_active_kcal(self):
        rec = self.recording_engine
        d = self.connection_engine.data
        if not rec.running or rec.start_monotonic is None:
            return None
        try:
            weight = None
            if self.health_use_weight.get() and self.health_weight_kg is not None:
                weight = float(self.health_weight_kg)
            if weight is None:
                weight = safe_float(self.profile_weight_kg.get())

            age = safe_float(self.profile_age.get())
            sex = self.profile_sex.get().strip().lower()
            bpm = safe_float(d.bpm)

            if weight is None or age is None or bpm is None:
                return None
            if sex not in ("male", "female"):
                return None
            if not (20 <= weight <= 300 and 12 <= age <= 100 and 35 <= bpm <= 230):
                return None

            elapsed_min = max(0.0, (time.monotonic() - rec.start_monotonic) / 60.0)
            if sex == "male":
                kj_per_min = -55.0969 + 0.6309*bpm + 0.1988*weight + 0.2017*age
            else:
                kj_per_min = -20.4022 + 0.4472*bpm - 0.1263*weight + 0.074*age

            value = max(0.0, kj_per_min / 4.184) * elapsed_min
    ...
    def update_status(self):
        ...
        if show_health_steps:
            ...
            if self.health_steps_today is not None:
                ...
                parts.append(f"-- {self.tr('Schritte')}")

            if self.health_show_spo2.get():
                if self.health_spo2_pct is not None:
                    parts.append(f"SpO₂ {de_number(self.health_spo2_pct, 1)} %")
                else:
                    parts.append("SpO₂ -- %")
            ...
            if self.health_weight_kg is not None:
                parts.append(f"{de_number(self.health_weight_kg, 1)} kg")

            text = f"{self.tr('Health Connect heute')}: " + " · ".join(parts)
        ...
        if rec.running:
            ...
            live_kcal = self.estimate_live_active_kcal()
            if live_kcal is not None:
                self.status_session_kcal.config(
                    text=f"~{de_number(live_kcal, 0)} kcal  ·  geschätzt"
                )
            else:
                self.status_session_kcal.config(
                    text="-- kcal  ·  Gewicht + Alter + Geschlecht + Puls nötig"
                )
            ...
            self.status_session_kcal.config(text="-- kcal  ·  geschätzt")
